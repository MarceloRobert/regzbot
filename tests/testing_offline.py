...
class Emaildir:
    ...
    def create_email(
        ...
        *,
        ...
        filename = os.path.join(self.directory, '%s.regzbot' % messageid.strip('<>'))
        if replyto:
            filename_replyto = os.path.join(
                self.directory, '%s.regzbot' % replyto.strip('<>')
            )
...
class TestingGitTree:
    def __init__(
        self, path_testdata, path_tmprepos, reponame, startdate, branchname='master'
    ):
        self._count = 0
    ...
    def __init_repo(self, path_tmprepos, reponame, startdate):
        ...
        if os.path.isdir(repodir):
            if not os.path.isdir(os.path.join(repodir, '.git')):
                logger.critical(
                    'Directory %s exist, but does not contain .git/. Aborting.'
                    % repodir
                )
                sys.exit(1)
        else:
    ...
    def __init_branch(self):
        filename = os.path.join(
            self.repo.working_dir, self._description + '-' + str(self._count)
        )
    ...
    def tag(
        self,
        tag,
        message="This is a tag for testing regzbot, the content doesn't matter.",
    ):
        # make sure our branch is checked out
        self.__checkout_branch()
...
def init_mailsdir(path_tmpmail):
    ...
    emaildirs['secondary'] = Emaildir(
        'linux-kernel@example.com', path_tmpmail, 'secondary'
    )
...
def offltest_0_1(funcname):
    replyto = 'test_0_0'
    logger.info(
        '%s: specifying the culprit for the regression created in %s'
        % (funcname, replyto)
    )
...
def offltest_0_2(funcname):
    replyto = 'test_0_0'
    logger.info(
        '%s: update title for the regression created in %s' % (funcname, replyto)
    )
    emaildirs['primary'].create_email(
        funcname,
...
def offltest_0_3(funcname):
    logger.info(
        '%s: create a second mainline regression and mark it immediately as duplicate'
        % funcname
    )
    emaildirs['primary'].create_email(funcname, '#regzbot introduced: v1.8..v1.9-rc1')
    replyto = funcname
...
def offltest_0_7(funcname):
    replyto = 'test_0_6'
    logger.info(
        '%s: mark the regression created in %s as resolved' % (funcname, replyto)
    )
    emaildirs['primary'].create_email(
        funcname, '#regzbot resolve: some reason', replyto=replyto
    )
    return ['mailchk']
...
def offltest_0_8(funcname):
    ...
    emaildirs['primary'].create_email(
        ...
        cc=emaildirs['secondary'].recipient,
    )
    return ['mailchk']
...
def offltest_0_13(funcname):
    ...
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
        '#regzbot backburner Some reason',
        replyto=replyto,
    )
    return ['mailchk']
...
def offltest_1_5(funcname):
    logger.info(
        '%s: create a mainline regression and have a commit refer to in in stable'
        % funcname
    )
    emaildirs['primary'].create_email(funcname, '#regzbot introduced: v1.8..v1.9-rc1')
...
def offltest_1_7(funcname):
    ...
    emaildirs['primary'].create_email(
        funcname,
        '#regzbot introduced: %s' % gittrees_testing['mainline'].hashes_known[-2],
    )
    return ['mailchk']
...
def offltest_1_11(funcname):
    ...
    testfix_subject = (
        'This is a test 123456789'  # that's the commit for the previous test
    )
...
def offltest_2_2(funcname):
    replyto = 'test_2_0'
    logger.info(
        '%s: remove the link to the regression created in %s' % (funcname, replyto)
    )
    emaildirs['primary'].create_email(
        funcname,
        '#regzbot unlink https://www.kernel.org/releases.html',
        replyto=replyto,
    )
    return ['mailchk']
...
def offltest_2_3(funcname):
    replyto = 'test_2_0'
    logger.info(
        '%s: refer to the regression created in %s on another mailing list'
        % (funcname, replyto)
    )
    emaildirs['secondary'].create_email(
        funcname,
...
def offltest_2_5(funcname):
    replyto = 'test_2_3'
    logger.info(
        '%s: add a reply to the thread %s that is now monitored' % (funcname, replyto)
    )
    emaildirs['secondary'].create_email(
        funcname,
...
def offltest_2_6(funcname):
    replyto = 'test_2_3'
    logger.info(
        '%s: use a rezbot comment in the thread %s that is now monitored'
        % (funcname, replyto)
    )
    emaildirs['secondary'].create_email(
        funcname,
        '#regzbot title new title set via a monitored thread',
        subject='%s: reply to the thread now monitored with a regzbot command'
        % funcname,
        replyto=replyto,
    )
    return ['mailchk']
...
def offltest_2_8(funcname):
    replyto = 'test_2_3'
    logger.info(
        '%s: add a reply to the thread %s that is now unmonitored' % (funcname, replyto)
    )
    emaildirs['secondary'].create_email(
        funcname,
        'Lorem ipsum dolor sit amet',
...
def offltest_2_11(funcname):
    ...
    emaildirs['secondary'].create_email(
        '%s_%s' % (funcname, subcounter),
        'Lorem ipsum dolor sit amet',
        subject='%s_%s: a patch to fix a regression missing a Link: tag'
        % (funcname, subcounter),
    )
    subcounter += 1
...
+	    !test_bit(DEVICE_STATE_STARTED, &rt2x00dev->flags))
    ...
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
        content,
        subject='[PATCH v2] %s_%s: add a mail with a simple patch'
        % (funcname, subcounter),
        replyto=replyto,
    )
...
def offltest_2_14(funcname):
    logger.info(
        '%s: create two regression and link to them using Link on another mailinglist'
        % (funcname)
    )
    ...
    emaildirs['primary'].create_email(
        ...
        'Link: https://lore.kernel.org/regressions/regzbot-testing-%s@example.com\nLink: https://lore.kernel.org/regressions/regzbot-testing-%s@example.com'
        % (replyto_1, replyto_2),
        subject='%s_%s: refer to this regression on another mainling list'
        % (funcname, subcounter),
    )
    return ['mailchk']
...
def offltest_2_15(funcname):
    logger.info(
        '%s: mark a regression that monitors some threads as a regression of another'
        % (funcname)
    )
...
def offltest_2_16(funcname):
    ...
    emaildirs['secondary'].create_email(
        funcname,
        'Closes: https://lore.kernel.org/regressions/regzbot-testing-%s@example.com'
        % replyto,
        subject='%s: refer to newly created regression on another mainling list'
        % funcname,
    )
    return ['mailchk']
...
def offltest_3_1(funcname):
    replyto = 'test_3_0'
    logger.info(
        '%s: specify the culprit for the regression created in %s' % (funcname, replyto)
    )
    emaildirs['primary'].create_email(
        funcname,
...
def offltest_3_2(funcname):
    replyto = 'test_3_0'
    logger.info(
        '%s: mark regression created in %s as fixed by and exiting commit'
        % (funcname, replyto)
    )
    emaildirs['primary'].create_email(
        funcname,
...
def offltest_3_4(funcname):
    ...
    emaildirs['primary'].create_email(
        funcname,
        '#regzbot introduced: %s'
        % gittrees_testing['linux-1.8.y'].hashes_known[1][0:11],
        replyto=replyto,
    )
    return ['mailchk']
...
def offltest_4_0(funcname):
    ...
    subcounter += 1
    logger.info(
        '%s: creating a mainline regression where the range spans two releases'
        % funcname
    )
    emaildirs['primary'].create_email(
    ...
    subcounter += 1
    logger.info(
        '%s: creating a mainline regression in the current cycle with open end)'
        % funcname
    )
    emaildirs['primary'].create_email(
...
def offltest_4_1(funcname):
    ...
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
        '#regzbot introduced: next-20190101..next-20190102',
    )
...
def offltest_4_2(funcname):
    ...
    subcounter += 1
    logger.info(
        '%s: creating a linux-stable regression (range starting with mainline)'
        % funcname
    )
    emaildirs['primary'].create_email(
...
def offltest_4_3(funcname):
    subcounter = 0
    logger.info(
        '%s_%s: creating a regressions that refers to non-existant tag'
        % (funcname, subcounter)
    )
    emaildirs['primary'].create_email(
...
def offltest_5_2(funcname):
    ...
    subcounter += 1
    logger.info(
        '%s_%s: try regzbot monitor with a unkown mailing list '
        % (funcname, subcounter)
    )
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
    ...
    subcounter += 1
    logger.info(
        '%s_%s: try regzbot unmonitor with a typo a unkown mailing list '
        % (funcname, subcounter)
    )
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
    ...
    subcounter += 1
    logger.info(
        '%s_%s: try regzbot unmonitor with a unkown mailing list '
        % (funcname, subcounter)
    )
    emaildirs['primary'].create_email(
        '%s_%s' % (funcname, subcounter),
