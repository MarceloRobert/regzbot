...
class RegressionMailReport(regzbot.RegressionFull):
    ...
    def compile(self, lastreport_gmtime):
        ...
        report.append(
            ...
            % (
                self._actim_report.repsrc.generic_name,
                self._actim_report.repsrc.entryid,
            )
        )
        report.append(
        ...
        if self.poked:
            statusline.append(
                '; poked %s days ago' % regzbot.days_delta(self.poked.gmtime)
            )
        statusline.append('.')
    ...
    def add_latestpatch(self, report):
        ...
        for actievent in reversed(self._actievents):
            ...
            report.append(
                '  %s days ago, by %s'
                % (regzbot.days_delta(actievent.gmtime), actievent.author)
            )
    ...
    def compile(cls):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            for counter, treename in enumerate(categories.keys()):
                report = cls.pagecreate(
                    categories[treename], treename, lastreport_msgid
                )
                ...
                filename = os.path.join(
                    tmpdirname, '%s-regzbotreport-%s' % (counter, treename)
                )
            ...
            answer = input(
                'Enter c to confirm you sent the report, anything else to abort: '
            )
            if answer.lower() != 'c':
                return
