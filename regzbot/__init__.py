...
class GitBranch:
    ...
    def get_by_treeid_branchname(gittreeid, name):
        ...
        dbcursor = DBCON.cursor()
        dbresult = dbcursor.execute(
            'SELECT * FROM gitbranches WHERE gittreeid=(?) AND name=(?)',
            (gittreeid, name),
        ).fetchone()
        if dbresult:
            return GitBranch(*dbresult)
    ...
    def is_abandoned(self, repo=None):
        ...
        date_offset = (
            int(datetime.datetime.now(datetime.timezone.utc).timestamp()) - 86400 * 63
        )
...
class GitTree:
        ...
        for gittree in GitTree.getall(FIXME=sortorder):
            ...
            for gitbranch in GitBranch.getall_by_gittreeid(gittree.gittreeid):
                ...
                if subject:
                    ...
                    hexsha = gitbranch.subject_exists(
                        subject, gittree=gittree, repo=repo
                    )
                    if hexsha:
                        yield gittree, gitbranch, hexsha
                        continue
    ...
    def get_by_name(treename):
        dbcursor = DBCON.cursor()
        dbresult = dbcursor.execute(
            'SELECT * FROM gittrees WHERE name=(?)', (treename,)
        ).fetchone()
        if dbresult:
            return GitTree(*dbresult)
        return None
    ...
    def search_references(cls, repsrc, regression, gmtime=None):
        ...
        for gittree in cls.getall(FIXME='ORDER BY priority ASC'):
            ...
            logger.debug(
                "[GitTree] Trying to find '%s' in gittree %s",
                searchstring,
                gittree.name,
            )
                    ...
                    if len(open_regressions) == 0:
                        for regression in RegressionBasic.get_all(only_unsolved=True):
                            if '..' not in regression.introduced:
                                open_regressions[regression.regid] = (
                                    regression.introduced[0:12]
                                )
...
class RegActivityMonitor:
    ...
    def get_by_entry(entry):
        dbcursor = DBCON.cursor()
        for dbresult in dbcursor.execute(
            'SELECT * FROM actmonitor WHERE entry=(?)', (entry,)
        ):
            return RegActivityMonitor(*dbresult)
...
class RegActivityEvent:
    ...
    def getall_by_actimonid(cls, actimonid):
        dbcursor = DBCON.cursor()
        for dbresult in dbcursor.execute(
            'SELECT %s FROM regactivity WHERE actimonid=(?)' % RegActivityEvent.DBCOLS,
            (actimonid,),
        ):
            yield cls(*dbresult)
    ...
    def present(entry, actimonid=None, regid=None, gitbranchid=None, subentry=None):
        ...
        if actimonid:
            ...
            if gitbranchid:
                ...
                dbresult = dbcursor.execute(
                    'SELECT * FROM regactivity WHERE regid=(?) AND entry=(?)',
                    (regid, entry),
                ).fetchone()
    ...
    def remove(actimonid=None):
        ...
        if dbresult is not None:
            ...
            logger.debug(
                '[db regactivity] deleted all lines where actimonid=%s)', actimonid
            )
...
class RegHistory:
    def __init__(
        self, regid, gmtime, entry, subject, regzbotcmd, gitbranchid, repsrcid, author
    ):
        self.regid = regid
        self.gmtime = gmtime
        self.entry = entry
    ...
    def present(entry, regid=None, repsrcid=None, gitbranchid=None):
        ...
        if gitbranchid and regid:
            ...
            dbresult = dbcursor.execute(
                'SELECT * FROM reghistory WHERE entry=(?) AND repsrcid=(?)',
                (entry, repsrcid),
            ).fetchone()
        else:
            dbresult = dbcursor.execute(
    ...
    def filed(regid):
        ...
        if not dbresult:
            dbresult = dbcursor.execute(
                'SELECT gmtime FROM reghistory WHERE regid=(?) ORDER BY gmtime',
                (regid,),
            ).fetchone()
        # fallback, in case history entry was not created yet
        if not dbresult:
            dbresult = dbcursor.execute(
                'SELECT gmtime FROM actmonitor WHERE regid=(?) ORDER BY gmtime',
                (regid,),
            ).fetchone()
        return dbresult[0]
...
class RegLink:
    ...
    def remove_entry(regid, repsrcid, entry):
        ...
        if dbcursor.execute(
            ...
            logger.debug(
                '[db reglinks] deleted (regid:%s, repsrcid:%s, entry:%s)'
                % (regid, repsrcid, entry)
            )
    ...
    def get_all(cls, regid, order='ASC'):
        dbcursor = DBCON.cursor()
        for dbresult in dbcursor.execute(
            'SELECT * FROM reglinks WHERE regid=(?) ORDER BY gmtime %s' % order,
            (regid,),
        ):
            yield cls(*dbresult)
...
class RegressionBasic:
    ...
    def __duplicate(self, rgzcmd, other):
        ...
        if self == older:
            ...
            older.add_history_event(
                rgzcmd,
                cmdline='duplicate: %s [implicit via duplicate]' % younger.web_url,
            )
        logger.info(
            'Regression(%s): now a duplicate of Regression(%s).',
            younger.web_url,
            older.web_url,
        )
    ...
    def cmd_from(self, rgzcmd, realname, username):
        ...
        logger.info(
            'Regression(%s)]: author is now %s, authormail now %s',
            self.web_url,
            realname,
            username,
        )
    ...
    def cmd_link(self, rgzcmd, url, description):
        ...
        logger.info(
            'regression[%s, "%s"]: added link %s' % (self.regid, self.subject, url)
        )
    ...
    def cmd_monitor(self, rgzcmd, url, description):
        ...
        logger.info(
            'regression[%s, "%s"]: started to monitor %s'
    ...
    def get_all(cls, order='regid', only_unsolved=False):
        ...
        if only_unsolved:
            ...
            for dbresult in dbcursor.execute(
                'SELECT %s FROM regressions ORDER BY %s'
                % (RegressionBasic.DBCOLS, order)
            ):
                yield cls(*dbresult)
    ...
    def get_by_regid(cls, regid, dbcursor=None):
        if not dbcursor:
            dbcursor = DBCON.cursor()
        dbresult = dbcursor.execute(
            'SELECT %s FROM regressions WHERE regid=?' % RegressionBasic.DBCOLS,
            (regid,),
        ).fetchone()
        if dbresult:
            return cls(*dbresult)
    ...
    def get_dupes(self, *, recursion_count=-1):
        ...
        dbcursor = DBCON.cursor()
        for dbresult in dbcursor.execute(
            'SELECT %s FROM regressions WHERE solved_duplicateof=(?)' % self.DBCOLS,
            (self.regid,),
        ):
    ...
    def find_topmost(self, *, recursion_count=-1):
        ...
        for regression in upper_regression.find_topmost(
    ...
    def fixes_expected():
        ...
        for dbresult in dbcursor.execute(
            ...
            pending.append(
                {
                    'regid': dbresult[0],
                    'solved_entry': dbresult[1],
                    'solved_subject': dbresult[2],
                }
            )
        return pending
    ...
    def dupof(self, tagload, gmtime, msgid, msgsubject, authorname, repsrcid):
        ...
        if not regression_other:
            ...
            RegHistory.event(
                ...
                authorname,
                repsrcid=repsrcid,
                regzbotcmd="introduced: %s [implicit, due to usage of 'dup-of']"
                % self.introduced,
            )

        self._dupof_direct(
            regression_other, gmtime, msgid, msgsubject, authorname, repsrcid
        )
    ...
    def lookup_fixedby_everywhere(self, commit_hexsha, subject, gmtime=None):
        for gittree, gitbranch, commit_hexsha in GitTree.commit_find_new(
            ...
            if culprit_gittree and gittree.priority > culprit_gittree.priority:
                ...
                continue
            self.fixedby_found(
                gittree, gitbranch, commit_hexsha, culprit_gittree, gmtime=gmtime
            )

    def fixedby_found(
        self, gittree, gitbranch, commit_hexsha, culprit_gittree=None, gmtime=None
    ):
        ...
        if (
            self.solved_reason == 'fixed'
            and self.solved_gitbranchid != gitbranch.gitbranchid
        ):
    ...
    def title(self, tagload):
        ...
        logger.info(
            'regression[%s, "%s"]: subject now "%s"', self.regid, self.subject, tagload
        )
    ...
    def _gettree_n_branch(introduced, gmtime=None):
        if '..' in introduced:
            ...
            if not range_end:
                ...
                commit = gitbranch_start.head_at_gmtime(
                    gmtime, repo=gittree_start.repo()
                )
            ...
            if (
                gitbranch_start
                and gitbranch_end
                and gitbranch_start.name == gitbranch_end.name
            ):
...
class RegressionFull(RegressionBasic):
    ...
    def _get_presentable(self, gitref, gittree=None, getversionline=None):
        ...
        if isdevcycle('indevelopment', point2):
            ...
            if isdevcycle('indevelopment', point1) or point1.startswith(
                LATEST_VERSIONS['latest']
            ):
                return combine(point1, point2), 'indevelopment'
        if isdevcycle('latest', point2):
            if isdevcycle('latest', point1) or point1.startswith(
                LATEST_VERSIONS['previous']
            ):
                return combine(point1, point2), 'latest'
...
class ReportSource:
    ...
    def get_by_identifier(identifier):
        dbcursor = DBCON.cursor()
        dbresult = dbcursor.execute(
            'SELECT * FROM reportsources WHERE identifiers LIKE (?)',
            ('%%%s%%' % identifier,),
        ).fetchone()
        if dbresult:
            return ReportSource(*dbresult)
    ...
    def get_by_url(cls, url):
        ...
        if not lowered_url[0].startswith('http'):
            ...
            if repsrc:
                ticketid = lowered_wo_protocol.removeprefix(
                    'bugzilla.kernel.org/show_bug.cgi?id='
                )
                repsrc = cls.get_byweburl(
            ...
            if lowered_url[3] == 'all':
                ...
                sys.exit(1)
            repsrc = cls.get_byweburl(
                'https://%s/%s/' % (lowered_url[2], lowered_url[3])
            )
            if repsrc:
...
def parse_link(url):
    ...
    if tmpstring.startswith('lore.kernel.org') or tmpstring.startswith(
        'lkml.kernel.org'
    ):
...
def basicressources_gittrees_setup(gittreesdir):
    ...
    for gittreedir in (
        ...
        if not basicressource_checkdir_exists(gitdir, create=False):
            logger.error(
                "Aborting, as the directory '%s' appears to not contain a git tree."
                % gittreedir
            )
            sys.exit(1)
...
def basicressources_get_dirs(
    databasedir=None, gittreesdir=None, websitesdir=None, tmpdir=None
):
    ...
    with tempfile.TemporaryFile(mode='w+t') as tmpfile_before:
        with tempfile.TemporaryFile(mode='w+t') as tmpfile_after:
            for msgid in msgids:
                ...
                if not regression:
                    logger.critical(
                        'Aborting, could not find any regression with msgid %s', msgid
                    )
                    sys.exit(1)
