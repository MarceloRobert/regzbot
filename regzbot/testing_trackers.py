...
def init(tmpdir):
    ...
    regzbot.basicressources_setup(
        tmpdir=tmpdir,
        gittreesdir=True,
        databasedir=os.path.join(tmpdir, 'db-trackertsts'),
    )
...
def trackertest_0_0(funcname):
    regzbot.ReportSource.add(
        ...
        '<unused>',
        lastchked=int(
            datetime.datetime.fromisoformat('2023-11-20T00:00:00.000Z').timestamp()
        ),
    )
    regzbot.ReportSource.add(
        ...
        '<unused>',
        lastchked=int(
            datetime.datetime.fromisoformat('2022-03-15T00:00:00.000Z').timestamp()
        ),
    )

    regzbot._TESTING['until'] = datetime.datetime.fromisoformat(
        '2023-11-20T11:35:00.000Z'
    )
