        ...
        if regzbot.is_running_citesting('offline'):
            ...
            for directory in regzbot._TESTING['emaildirs']:
                ...
                for mboxmsg in mailbox.mbox(filename):
                    yield email.message_from_bytes(
                        mboxmsg.as_bytes(), policy=email.policy.default
                    )
            if not found_something:
            ...
            with tempfile.NamedTemporaryFile() as tmpfile:
                ...
                try:
                    ...
                    logger.critical(
                        '[lore] failed to download thread from %s: %s', url, err
                    )
                    raise regzbot.RepDownloadError
...
class LoRepAct(regzbot.ReportActivity):
    def __init__(self, reptrd, lo_activity):
        ...
        if reptrd.id == lo_activity.id:
            ...
            self.reptrd = LoRepTrd(
                self.repsrc, self.lo_thread, lo_activity=self.lo_activity
            )
        self.id = None
...
class LoRepSrc(ReportSource):
    ...
    def update(self):
        if regzbot.is_running_citesting('offline'):
            ...
            filenames = sorted(
                pathlib.Path(self.serverurl).iterdir(), key=os.path.getmtime
            )
            for file in filenames:
                ...
                for mboxmsg in mailbox.mbox(file):
                    ...
                    if regzbot.RecordProcessedMsgids.check_presence(
                        lo_retrd.id, lo_retrd.gmtime
                    ):
                        continue
            ...
            for id, over in lorenntp._over(self.lastchked + 1, id_last):
                ...
                if regzbot.RecordProcessedMsgids.check_presence(msgid, gmtime):
                    logger.debug(
                        '[lore] skipping "%s", we already encountered it it', msgid
                    )
                    continue

                msg = lorenntp._article(id, self.serverurl)
                if 'subject' in msg and msg['subject'].startswith(
                    regzbot.REPORT_SUBJECT_PREFIX
                ):
                    logger.debug(
                        "[lore] skipping mail %s, as it's a report we send", msgid
                    )
                    continue
                if 'from' in msg:
                    if (
                        ...
                        logger.debug(
                            "[lore] skipping mail %s, as it's a bugzilla mail", msgid
                        )
                        continue
                lo_thread = LoreThread(msg=msg)
                lo_retrd = LoRepTrd(self, lo_thread)
...
class LoRepTrd(ReportThread):
    ...
    def update(self, since, until, *, actimon=None, triggering_repact=None):
        ...
        #  processed earlier
        try:
            for activity in self._lo_thread.activities(
                msgid=self.id, since=since, until=until
            ):
                ...
                regzbot.RecordProcessedMsgids.check_presence(
                    activity.id, gmtime=activity.gmtime
                )
