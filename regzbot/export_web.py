    ...
    def html(self, yattagdoc, regression):
        ...
        if self.author:
            with yattagdoc.tag('div', style='padding-left: 3em;'):
                ...
                if (
                    self._for_regression
                    and not regression.regid == self._for_regression.regid
                ):
                    yattagdoc.text(' [')
                    with yattagdoc.tag(
                        'a',
                        href='../regression/%s/'
                        % self._for_regression._actim_report.entry,
                    ):
    ...
    def html(self, yattagdoc, regression):
        ...
        with yattagdoc.tag('div', style='padding-left: 2em;'):
            ...
            if (
                ...
                with yattagdoc.tag(
                    'a',
                    href='../regression/%s/%s/' % (repsrc.generic_name, repsrc.entryid),
                ):
                    yattagdoc.text('via dup')
                yattagdoc.text(']')
                ...
                with yattagdoc.tag('li'):
                    if self._introduced_url:
                        ...
                        if self._introduced_presentable:
                            with yattagdoc.tag('div'):
                                yattagdoc.text(
                                    '(within %s)' % self._introduced_presentable
                                )
        ...
        def cell2(yattagdoc):
            ...
            def __resolution():
                if self.solved_reason == 'fixed':
                    ...
                    self.solved_reason == 'resolved' or self.solved_reason == 'invalid'
                ):
            ...
            with yattagdoc_line.tag(
                ...
                with yattagdoc_line.tag(
                    'summary', style='list-style-position: outside;'
                ):
                    ...
                    for actireport in actireports_sorted:
                        with yattagdoc.tag(
                            'a',
                            href=regzbot.ReportSource.get_by_id(
                                actireport.repsrcid
                            ).url(actireport.entry),
                        ):
                    ...
                    with yattagdoc.tag('div'):
                        ...
                        if self.solved_reason or self.solved_duplicateof:
                            if self.solved_reason:
                                ...
                                with yattagdoc.tag(
                                    'mark', style='background-color: #D0D0D0;'
                                ):
                                    yattagdoc.text('[')
                            ...
                            if self.solved_duplicateof:
                                ...
                                regression_duplicateof = self.get_by_regid(
                                    self.solved_duplicateof
                                )
                                __dup_report_repsrc = (
                                    regression_duplicateof._actim_report.repsrc
                                )
                                with yattagdoc.tag(
                                    'a',
                    ...
                    with yattagdoc.tag('div'):
                        ...
                        with yattagdoc.tag('div', style='padding-left: 3em;'):
                            ...
                            if (
                                ...
                                and not self._solved_entry_presentable
                                == self.solved_entry[:12]
                            ):
                                yattagdoc.text(
                                    ' in %s' % self._solved_entry_presentable
                                )
                else:
                    latest_shown = False
                    earlier_patches = 0
                        ...
                        if not latest_shown:
                            ...
                            with yattagdoc.tag('div', style='padding-left: 3em;'):
                                ...
                                if (
                                    ...
                                    PatchKind.DIFF | PatchKind.SUBJECT
                                ) in actievent.patchkind:
                ...
                with yattagdoc_line.tag('p'):
                    ...
                    with yattagdoc_line.tag(
                        'ul', style='padding-left: 5px; margin-top: -1em;'
                    ):
                ...
                with yattagdoc.tag('p'):
                    ...
                    with yattagdoc_line.tag(
                        ...
                        with yattagdoc.tag('li'):
                            ...
                            if self._introduced_url:
                                commitsummary = regzbot.GitTree.commit_summary(
                                    self.introduced
                                )
                                yattagdoc.text(
                                    'Fixes: %s ("%s")'
                        ...
                        for regression in self, *self._dupes:
                            ...
                            reportedlink = regzbot.ReportSource.get_by_id(
                                actireport.repsrcid
                            ).url(actireport.entry, redirector=True)
...
class RegExportWeb:
    ...
    def outpage_header(yattagdoc, htmlpages, pagename, relpath=''):
        yattagdoc.asis(
            '<script src="%s../relativetime.js" type="text/javascript"></script>'
            % relpath
        )
    ...
    def outpage_table_span(
        ...
        with yattagdoc.tag('tr'):
            ...
            with yattagdoc.tag(
                ...
                style='text-align: left;  padding-bottom: 1em;',
            ):
    ...
    def outpage_footer(yattagdoc, count):
        ...
        with yattagdoc.tag('p', style='text-align: center'):
            ...
            with yattagdoc.tag(
                'a',
                href='https://gitlab.com/knurd42/regzbot/-/blob/main/docs/getting_started.md',
            ):
                yattagdoc.text('getting started guide')
            yattagdoc.text(' or its ')
            with yattagdoc.tag(
                'a',
                href='https://gitlab.com/knurd42/regzbot/-/blob/main/docs/reference.md',
            ):
        ...
        if count > 0:
            with yattagdoc.tag('p', style='text-align: center'):
                ...
                with yattagdoc.tag('a', href='../unhandled/'):
                    if count == 1:
                        yattagdoc.text(
                            '%s event occurred that regzbot was unable to handle'
                            % count
                        )
                    else:
                        yattagdoc.text(
                            '%s events occurred that regzbot was unable to handle'
                            % count
                        )
                yattagdoc.text(']')
    ...
    def create_individual_page(cls, htmlpages, unhandled_count, regression):
        ...
        with yattagdoc.tag('body'):
            ...
            cls.outpage_write(
                'regression/%s/%s'
                % (regression.repsrc.generic_name, regression.repsrc.entryid),
                yattagdoc,
            )
            ...
            with yattagdoc.tag('table', style='width:100%;'):
                for category in categories.keys():
                    ...
                    if not categories[category]['entries']:
                        cls.outpage_table_span(
                            yattagdoc, 'none known by regzbot', tablecolumns
                        )
                    # add html
    ...
    def create_scriptfile_reldate():
        with open(
            os.path.join(regzbot.WEBPAGEDIR, 'relativetime.js'), 'w'
        ) as outputfile:
...
""")
    ...
    def categorize(cls, regressionlist):
        if regzbot.LATEST_VERSIONS['indevelopment'] == False:
            ...
            indevelopment_descriptive = (
                '%s-rc' % regzbot.LATEST_VERSIONS['indevelopment']
            )
            ...
            'mainline': {
                ...
                'identified_latest': {
                    ...
                    % (
                        ...
                        regzbot.LATEST_VERSIONS['latest'],
                    ),
                    'entries': list(),
                },
                'identified_old': {
        ...
        for regression in regressionlist:
            ...
            if regression.solved_reason == 'inconclusive':
                ...
                categories[regression.treename]['backburner']['entries'].append(
                    regression
                )
                ...
                if regression.identified:
                    categories[regression.treename]['identified']['entries'].append(
                        regression
                    )
                else:
                    categories[regression.treename]['default']['entries'].append(
                        regression
                    )
                ...
                if regression.versionline == 'indevelopment':
                    ...
                    categories[regression.treename]['identified_latest'][
                        'entries'
                    ].append(regression)
                elif regression.versionline == 'latest' and last_activity_days < 21:
                    categories[regression.treename]['unidentified_latest'][
                        'entries'
                    ].append(regression)
                elif regression.identified:
                    categories[regression.treename]['identified_old']['entries'].append(
                        regression
                    )
                elif last_activity_days < 21:
                    categories[regression.treename]['unidentified_old'][
                        'entries'
                    ].append(regression)
                else:
                    categories[regression.treename]['default']['entries'].append(
    ...
    def compile(cls):
        ...
        for pagename in categories.keys():
            cls.createpage_compilation(
                htmlpages, unhandled_count, categories[pagename], pagename
            )

        # create default
        with open(os.path.join(regzbot.WEBPAGEDIR, 'index.html'), 'w') as outputfile:
            outputfile.write(
                "<head><meta http-equiv='refresh' content='0; URL=mainline/'></head>"
            )
        ...
        with open(
            os.path.join(regzbot.WEBPAGEDIR, 'regressions.json'), 'w'
        ) as jsonfile:
